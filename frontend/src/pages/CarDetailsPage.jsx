      ...
      <PageTransition>
        <section className="mx-auto max-w-7xl px-4 pb-24 pt-32 sm:px-6 lg:px-8">
          <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="h-[62vh] min-h-96 animate-pulse rounded-lg bg-slate-800" />
            <CarCardSkeleton />
          </div>
        </section>
      </PageTransition>
    ...
    return (
      <PageTransition>
        <section className="mx-auto max-w-4xl px-4 py-36 text-center">
          ...
          <Button to="/inventory" className="mt-6">
            Back to inventory
          </Button>
        </section>
      </PageTransition>
  ...
  return (
    <PageTransition>
      <section className="mx-auto max-w-7xl px-4 pb-24 pt-28 sm:px-6 lg:px-8">
        <Link
          to="/inventory"
          className="mb-6 inline-flex items-center gap-2 text-sm text-muted hover:text-platinum"
        >
          <ArrowLeft size={16} />
          Back to inventory
        </Link>
        ...
        <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
          <div>
            <div className="relative overflow-hidden rounded-lg border border-white/10">
              <img
                className="h-[62vh] min-h-96 w-full object-cover"
              ...
              <div className="absolute left-5 top-5 flex flex-wrap gap-2">
                <Badge
                  tone={
                    car.availability === 'Available' ? 'success' : 'warning'
                  }
                >
                  {car.availability}
                </Badge>
          ...
          <aside className="h-fit rounded-lg border border-white/10 bg-slate-900 p-6 lg:sticky lg:top-24">
            ...
            <div className="my-6 border-y border-white/10 py-6">
              ...
              <p className="mt-1 text-4xl font-bold text-platinum">
                {formatCurrency(car.price)}
              </p>

              <p className="mt-2 text-sm text-gold">
                Estimated EMI {formatCurrency(emiFromPrice(car.price))}/month
              </p>
            </div>
          ...
          <div className="space-y-8">
            <section className="rounded-lg border border-white/10 bg-slate-900 p-6">
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                {specs.map(([label, value, Icon]) => (
                  <div
                    key={label}
                    className="flex items-center gap-3 rounded-lg border border-white/10 bg-slate-800 p-4"
                  >
            ...
            <section className="rounded-lg border border-white/10 bg-slate-900 p-6">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {car.features?.map((feature) => (
                  <span
                    key={feature}
                    className="flex items-center gap-2 rounded-lg border border-white/10 bg-slate-800 px-4 py-3 text-sm"
                  >
            ...
            <section className="rounded-lg border border-white/10 bg-slate-900 p-6">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                {[
                  'Accident-free body report',
                  'Service records verified',
                  'RC and loan status checked',
                ].map((item) => (
                  <div
                    key={item}
                    className="rounded-lg border border-emerald-300/20 bg-emerald-950 p-4 text-sm text-emerald-100"
                  >
                    <BadgeCheck className="mb-3" size={22} />
                    {item}
                  </div>
                ))}
              </div>
          ...
          <div className="rounded-lg border border-white/10 bg-slate-900 p-6">
            ...
            <div className="mt-5 space-y-4 text-sm">
              <div className="flex justify-between">
                <span className="text-muted">Ex-showroom equivalent</span>
                <span>{formatCurrency(Math.round(car.price * 1.08))}</span>
              </div>

              <div className="flex justify-between">
                <span className="text-muted">DZ AutoElite listed price</span>
                <span>{formatCurrency(car.price)}</span>
              </div>

              <div className="flex justify-between">
                <span className="text-muted">Documentation support</span>
                <span>Included</span>
              </div>
              ...
              <div className="flex justify-between border-t border-white/10 pt-4 font-semibold">
                ...
                <span className="text-gold">
                  {formatCurrency(emiFromPrice(car.price))}
                </span>
              </div>
            </div>
          </div>
